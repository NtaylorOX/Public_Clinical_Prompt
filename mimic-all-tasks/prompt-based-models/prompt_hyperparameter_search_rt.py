...
parser.add_argument("--save_dir",
                    default = "/mnt/sdg/niallt/saved_models/mimic-tasks/prompt-based-models/ray_tune_results/",
                    type=str,
...
if args.tune_plm == True:
    ...
    logs_dir = f"{args.project_root}/raytune_results/{args.dataset}/{args.model_name_or_path}_temp{args.template_type}{args.template_id}_verb{args.verbalizer_type}{args.verbalizer_id}_{args.training_size}_{args.few_shot_n}/{version}"

else:
    logger.warning("Freezing the plm")
    freeze_plm = True
    # set checkpoint, logs and params save_dirs    
    logs_dir = f"{args.project_root}/raytune_results/{args.dataset}/frozen_plm/{args.model_name_or_path}_temp{args.template_type}{args.template_id}_verb{args.verbalizer_type}{args.verbalizer_id}_{args.training_size}_{args.few_shot_n}/{version}"

# set up tensorboard logger
writer = SummaryWriter(logs_dir)

def train_mimic( num_epochs, mode = "train", 
...
analysis = tune.run(
    ...
    local_dir = f"{args.save_dir}",
    name=f"tune_mimic_{args.dataset}"
