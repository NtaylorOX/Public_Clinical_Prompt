...
class TemplateGenerator:
    ...
    def _register_buffer(self, data):
        ...
        print(f"inside register buffer")
        print(f"input ids buffer: {self.input_ids_buffer}")
        print(f"labels buffer: {self.labels_buffer}")
    ...
    def convert_template(self, generated_template: List[str], original_template: List[Dict]) -> str:
        ...
        i = 0
        part_id = 0
        print(f"inside convert template, generated template is: {generated_template}")
    ...
    def _get_templates(self):
        ...
        for i in tqdm(range(self.max_length - 2)):
            ...
            for item in current_output:
                ...
                tokens_ = self.tokenizer.convert_ids_to_tokens(ids)
                # print(f"word ids decoded: {tokens_}")
                for word_id in ids:
                    ...
                    if check:
                        ...
                        # print(f"current new output text is: {[self.tokenizer.convert_ids_to_tokens(item['output']) for item in new_item]}")
            if len(new_current_output) == 0:
                print("new_current_output was of length 0")
                break

            # print(f"all output before subsetting: {[self.tokenizer.convert_ids_to_tokens(item['output']) for item in new_current_output]}")   
            ...
            print(f"final current output to be returned is: {[self.tokenizer.convert_ids_to_tokens(item['output']) for item in current_output]}")
...
class T5TemplateGenerator(TemplateGenerator):
    ...
    print(f"inside t5 template generator")
    def __init__(self, 
